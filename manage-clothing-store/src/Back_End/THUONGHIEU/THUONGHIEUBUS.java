...
public class THUONGHIEUBUS {
    ...
    public void insertDTO(ArrayList<ArrayList<Object>> data) {
         ArrayList<THUONGHIEU> listKM = new ArrayList<>();
        for (ArrayList<Object> voucher : data) {
            String maTH = voucher.get(1).toString();
            String tenTH = voucher.get(2) + "";
            String trangthai = "ĐANG HOẠT ĐỘNG";
            THUONGHIEU kmDTO = new THUONGHIEU(maTH, tenTH, trangthai);
            listKM.add(kmDTO);
        }
        thDAO.insertArray(listKM);
    }
